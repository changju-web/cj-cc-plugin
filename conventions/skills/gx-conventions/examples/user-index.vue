...
const columns = generateTableColumns(UserTableModel, [
  ...
  { prop: 'status', type: 'dict-tag', props: { dictCode: 'system-status' }, width: 90 },
...
const userAddRef = ref<InstanceType<typeof UserAdd> | null>()
const UserAreaRef = ref<InstanceType<typeof UserArea> | null>()
const tenantAllotRef = ref<InstanceType<typeof TenantAllot> | null>()
...
const handleBatchDel = async () => {
  ...
  setLoading(true)
  try {
    const { message } = await User.batchRemove(selectionRows.map((item) => item.id))
    ElMessage.success(message)
    clearSelectionRows()
    reloadList()
  } catch (error) {
...
const handleResetPassword = async () => {
  ...
  setLoading(true)
  try {
    const { message } = await User.resetPassword(selectionRows.map((item) => item.id))
...
<template>
  <div class="user-manage">
    ...
    >
      ...
      <template #action-bar>
        <ElButton v-perm="['system:user:add']" type="primary" @click="handleAdd">
          <Icon icon="ep:plus" class="mr-5px" /> 新增
        </ElButton>
        <ElPopconfirm title="是否批量删除勾选用户?" placement="right" @confirm="handleBatchDel">
          <template #reference>
      ...
      <template #action="{ row }">
        <ElButton v-perm="['system:user:edit']" type="primary" link @click="handleEdit(row)">
          编辑
        </ElButton>
        <ElButton type="primary" link @click="handleArea(row)"> 管理区域 </ElButton>
        <ElPopconfirm title="是否删除该用户?" placement="left" @confirm="handleDel(row)">
          <template #reference>
            <ElButton v-perm="['system:user:del']" type="danger" link> 删除 </ElButton>
          </template>
        </ElPopconfirm>
        <ElButton
    ...
    <TenantAllot ref="tenantAllotRef" v-model="tenantAllotVisible" @reset-table="reloadList" />
  </div>
</template>
